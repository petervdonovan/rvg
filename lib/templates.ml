...
let rec parseToken e = let e', meta = e in
  match e' with
  | Template (exprs, env) -> if List.length exprs = 0 then None, Some env, None else
    ...
    | Some (Asm token, meta') -> (
      ...
      | Some remainder -> Some (Asm token, meta'), chosenEnv, Some (Template (remainder :: tail, env), meta )
      | None -> (if List.length tail = 0 then Some (Asm token, meta'), chosenEnv, None
        else let remainder = Template (tail, env), meta in
          if not (startsWithWord remainder) then Some (Asm token, meta'), chosenEnv, Some remainder
          else let rest, _, remainder' = parseToken remainder in
          (match rest with
          | Some (Asm rest, meta'') ->
              Some (Asm (token ^ rest), { attrs = Attributes.union meta''.attrs meta'.attrs; r = meta'.r }), chosenEnv, remainder'
          | _ -> Some (Asm token, meta'), chosenEnv, Some remainder)))
    | Some _ -> token, chosenEnv, (match remainder with
...
let parseTokenExpectingString env e =
  ...
  match token with
  | Some (Asm s, meta) -> Some ((s, meta.r), Option.value env' ~default:env), e'
  | Some (Integer i, meta) -> Some ((string_of_int i, meta.r), Option.value env' ~default:env), e'
  | None -> None, e'
...
let get2Tokens env e =
  ...
  | Some a, Some e' -> (match parseTokenExpectingString env e' with
    | Some b, e'' -> Some (a, b), e'' | _ -> None, None) | _ -> None, None
let get3Tokens env e =
  match get2Tokens env e with
  | Some (a, b), Some e' -> (match parseTokenExpectingString env e' with
...
let get3TokensWithThirdTokenInParens env e =
  ...
  | Some (regR, immR, ((openParen, _), _)), Some e' -> (
    ...
    else match get2Tokens env e' with
