...
let currentMode = match Array.get Sys.argv 1 with
  ...
  | "definition" ->
    ...
    Definition ({ zeroBasedLine; zeroBasedCol }: CharStream.position)
  | _ -> Execution
let sideEffectsAllowed =
  Array.get Sys.argv 1 <> "tokens"
let print arg =
  (match arg with
...
let announceToken expr (r: CharStream.range) =
  ...
  match kind with
  | Some kind ->
    if (Array.get Sys.argv 1 = "tokens") then print_endline ("{\"kind\": \"" ^ kind ^ "\", \"modifier\": \"" ^ modifier ^ "\", \"range\": " ^ rangeToList r ^ " }")
  | None -> ()
let reportResolvedName range definition =
  ...
  | Definition p -> if CharStream.contains p range then
    print_endline ("{\"range\": " ^ (definitionRange |> rangeToList) ^ "}") else ()
  | _ -> ()
