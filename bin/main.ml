let evaluate env f s =
  let evaluated =
    s |> Rvg.Ast.parseTopLevel Rvg.Std.stdFun |> Rvg.Eval.evalExpr env |> fst
  in
  Rvg.Eval.Environment.add
...
let evaluate env fname =
  ...
  if List.length parts > 1 then
    ...
    evaluate env name
      (Rvg.CharStream.fromString name (String.concat "=" contents))
  else evalFile env fname

let printMessageRange s r =
  print_endline (Rvg.CharStream.rangeToString r ^ ": " ^ s)
